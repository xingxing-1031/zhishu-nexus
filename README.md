...
- 当前最终评测记录见 `evaluation/final/release-manifest-20260829-final.json`：100 条 development（70 条业务真实链路、30 条 Runtime 专项分层）与 30 条一次性 holdout。业务 development 契约通过 48/70（68.57%），安全拒答 8/8，业务非失败 85.48%，P50/P95 为 11.088s/27.988s；15 条 live holdout 通过 1/15（6.67%），Runtime 探针另存原始报告，失败样本和外部工具依赖保留在分母。
- 历史报告仍保留用于升级对照；简历只引用 `evaluation/final/` 中的最终发布版报告，不混用旧基线数字
