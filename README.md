...
- 当前计划任务：`W1-4` FastAPI 基础查询接口与第一轮代码审查
- 总体进度：`3 / 32` 个项目里程碑
- 已完成：项目初始化、Pydantic 领域模型、零售 ER 模型和核心 SQL 练习
- 自动化测试：`12 passed`
- 当前边界：SQL 已完成逻辑审查，尚未在真实 PostgreSQL 和种子数据上执行
...
1. 完成 FastAPI 基础查询接口与接口测试。
2. 使用 PostgreSQL 建表、迁移并导入可复现的种子数据。
3. 将现有 SQL 放入真实数据库执行并保存验证证据。
