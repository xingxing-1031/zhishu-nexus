...
- “知枢 Nexus”品牌发布基线提交：`6d78af0`。
- GitHub Actions `CI`：成功，运行编号 `31838590245`。
- GitHub Actions `Deploy VPS release`：成功，运行编号 `31838590214`。
- 生产发布脚本最终输出并通过 `/ready` 检查。
- GitHub 仓库、本地目录、FastAPI 标题、前端包名、登录页、工作台和 AI 头像已统一为“知枢 Nexus / zhishu-nexus”；VPS 内部目录继续保留旧路径用于发布兼容。
- 公网 `/health` 返回 `ok`，`/ready` 返回 `ready`；桌面 1440×900 与手机 390×844 视口均已验证新品牌，无横向溢出，手机输入区固定在视口底部。
