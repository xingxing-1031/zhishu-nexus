...
我正在开发一个面向零售运营的可审计数据分析 Agent。W4-1 建立了版本化指标字典和 Schema 目录，W4-2 则把这些静态知识接入 LangGraph 的真实检索节点。

检索节点接收已经通过 Pydantic 校验的 `AnalysisPlan`。计划包含指标、维度和筛选：指标决定算什么，维度决定按什么分组，筛选决定哪些原始业务记录参与计算。工具必须同时分析三者，因为维度和筛选可能引入指标定义之外的新表。

工具首先从 `MetricCatalog` 找到指标的具体版本和业务公式，然后检查所选维度是否在指标支持范围内。如果业务口径不支持这个组合，工具会提前拒绝。例如当前退款表只记录订单级退款，没有退款明细，因此退款金额不支持商品维度。虽然数据库可以通过订单连接到商品，但强行连接可能把一笔退款重复匹配给多个商品，得到错误结果。

校验通过后，工具收集必要表，并只使用 `SchemaCatalog` 中批准的 JOIN 寻找最短连接路径。按商品统计销售额需要 `orders`、`order_items` 和 `products`，以及订单到明细、商品到明细的两条 JOIN；按退款状态统计退款金额只需要 `refunds`，不需要 JOIN。

检索结果统一转换为带 `source_id` 的 `RetrievalEvidence`，由检索节点写入 `AnalysisState.retrieved_context`，供后续 SQL 生成节点使用，也为审计保留具体依据。

当前实现是确定性的目录检索，不是向量 RAG，也没有接入真实大模型。新增 6 项测试覆盖最小证据、JOIN 选择、去重、非法维度和状态写入，全量回归为 121 项通过。下一阶段会评估检索结果，之后才让真实模型从自然语言生成计划和 SQL。
