...
WITH generated_orders AS (
    SELECT
        series,
        'DEMO-ORD-' || lpad(series::text, 3, '0') AS order_id,
        CASE series % 4
            WHEN 0 THEN '京东'
            WHEN 1 THEN '淘宝'
            WHEN 2 THEN '抖音'
...
SELECT
    'DEMO-ITEM-' || lpad(series::text, 3, '0') || '-' || item_no,
    order_id,
...
SELECT
    'DEMO-REF-' || lpad(series::text, 3, '0'),
    'DEMO-ORD-' || lpad((series * 5)::text, 3, '0'),
    (40 + series * 35)::NUMERIC(12, 2),
    CASE series % 5
        WHEN 0 THEN '商品与描述不符'
        WHEN 1 THEN '物流破损'
    ...
    CASE series % 4
        ...
        ELSE 'rejected'
    END,
    CURRENT_TIMESTAMP - make_interval(days => (series * 5) % 70)
