...
def main() -> None:
    ...
    parser.add_argument("--dataset", default="evaluation/zhishu_development.jsonl")
    parser.add_argument("--output-dir", default="evaluation/reports")
    args = parser.parse_args()
    dataset_path = Path(args.dataset)
    cases = load_zhishu_cases(dataset_path.read_text(encoding="utf-8").splitlines())
    ...
    def execute(case: ZhishuEvaluationCase) -> AgentResponse:
        response = client.post(
            "/agent/run",
            json={
                "request_id": f"ZHISHU-EVAL-{case.case_id}",
                "conversation_id": f"zhishu-eval-{case.case_id}",
                "user_id": user_id,
    ...
    try:
        report = evaluate_zhishu_cases(cases, execute, dataset=dataset_path.name)
    finally:
        client.close()
    output_dir = Path(args.output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    timestamp = datetime.now(UTC).strftime("%Y%m%dT%H%M%SZ")
    output = output_dir / f"zhishu-development-{timestamp}.json"
    output.write_text(
